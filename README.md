...
###  ⚠ Warning
Installing a new script usually overwrites the previous one, unless you manually change the installation address in the .asm file and recompile it. The script selector also has **limited space**, so not all installable scripts can fit at the same time. In order to get access to every script at any moment in the game, [BBMenu](https://github.com/M4n0zz/BBMenu) is now available.
